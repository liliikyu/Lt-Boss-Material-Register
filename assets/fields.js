(() => {
  ...
  function regionFor(name){
    const n=normRegionName(name).replace(/\bfield\b$/,"field");
    if(REGION_LOOKUP.has(n)) return REGION_LOOKUP.get(n);
    // A few wiki labels vary slightly from the map labels.
    const aliases={
      "eidos the sanctuary":"Freios",
      "linlos sacred place":"Western Freios",
      "la chouette slums":"Western Freios",
      "sacred place linlos":"Western Freios",
      "deborah snow field":"Eastland",
      "under city construction site":"Eastland"
    };
    return aliases[n] || "Other / Unmapped";
  }
