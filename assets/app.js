(() => {
  ...
  function updateProgressSummary() {
    ...
    const monsterBody = $("monster-progress-summary-body");
    if (!tbody && !monsterBody) return;
    ...
    if (monsterBody) {
      monsterBody.innerHTML = `<tr>
        <th scope="row">Monster Illustrations</th>
        <td>${monsterTotal}</td>
        <td>${monsterDone}</td>
        <td class="progress-left${monsterLeft === 0 && monsterTotal > 0 ? " complete" : ""}">${monsterLeft === 0 && monsterTotal > 0 ? "✓" : monsterLeft}</td>
      </tr>`;
    }
  ...
  function render() {
    ...
    $("dungeon-grid").innerHTML = visible.map((dungeon) => {
      ...
      const monsterRows = illustrations.length ? illustrations.map((monster) => {
      ...
      }).join("") : '<div class="monster-illustration-empty">No Monster Illustrations listed for this dungeon.</div>';
      const monsterSectionHtml = `<section class="monster-illustration-section">
      ...
      const titleNote = `<div class="card-note"><strong>Title:</strong>${titleText ? ` ${titleText}` : ""}</div>`;

      const dungeonKey = normalize(dungeon.name);
      const hasDrops = (dungeon.items || []).length > 0;
      const isExpandable = hasDrops || illustrations.length > 0 || (dungeon.titleNames || []).length > 0;
      ...
      const summaryHtml = `<div class="dungeon-summary" aria-hidden="${isExpanded ? "true" : "false"}">
        ...
        ${trackableItems.length ? `<span class="summary-progress"><strong>Done</strong> ${completedCount}/${trackableItems.length}</span>` : ""}
      </div>`;
