    ...
    $("dungeon-grid").innerHTML = visible.map((dungeon) => {
      ...
      const headerHtml = isExpandable ? `<button class="dungeon-toggle" type="button" aria-expanded="${isExpanded ? "true" : "false"}" title="${isExpanded ? "Return to summary" : "Show full list"} ${esc(dungeon.name)}">
        <div class="dungeon-title-wrap">
          ...
          <div class="dungeon-card-submeta">${illustrations.length} illustrations · ${trackableItems.length} codex</div>
        </div>
      ...
      const cardContentHtml = `
        ...
        ${monsterSummaryHtml}
        ${summaryHtml}
        ${isExpandable ? `<div class="dungeon-body">${monsterSectionHtml}${columnsHtml}<section class="dungeon-conquest-placeholder"><div class="dungeon-conquest-placeholder-head"><strong>Dungeon Conquest</strong><span class="coming-soon-badge">Coming Soon</span></div></section></div>` : ""}
        ${titleNote}
      `;
