(() => {
  ...
  function render() {
    ...
    $("dungeon-grid").innerHTML = visible.map((dungeon) => {
      ...
      const titleNote = titleText ? `<div class="card-note"><strong>Title:</strong> ${titleText}</div>` : "";

      const dungeonKey = normalize(dungeon.name);
      const hasDrops = hasAnyItems;
      const isExpandable = hasDrops || illustrations.length > 0 || Boolean(titleText);
      ...
      const overallHtml = overallTotal ? `<div class="dungeon-overall-progress" data-overall-progress>
      ...
      const monsterSummaryHtml = illustrations.length ? `<div class="monster-summary" aria-hidden="${isExpanded ? "true" : "false"}">
      ...
      const summaryHtml = hasAnyItems ? `<div class="dungeon-summary" aria-hidden="${isExpanded ? "true" : "false"}">
