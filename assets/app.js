(() => {
    ...
    $("dungeon-grid").innerHTML = visible.map((dungeon) => {
      ...
      const renderGroup = (key, label) => {
        ...
        const rows = items.length ? items.map((item) => {
          ...
          const trackable = Boolean(item.codex);
      ...
      const trackableItems = (dungeon.items || []).filter((item) => item.codex);
  ...
  document.addEventListener("click", (event) => {
    ...
    if (progressPanel.contains(event.target) || progressToggle.contains(event.target)) return;
    progressPanel.classList.add("hidden");
    progressToggle.setAttribute("aria-expanded", "false");
    progressToggle.setAttribute("aria-label", "Open Codex Progress");
  });
