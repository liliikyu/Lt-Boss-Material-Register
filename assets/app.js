  ...
  function render() {
    ...
    $("dungeon-grid").innerHTML = visible.map((dungeon) => {
      ...
      const uniqueLootSectionHtml = uniqueLoot.length ? `<section class="unique-loot-section">
        <div class="unique-loot-head"><strong>Unique Loot</strong><span class="unique-loot-count">${uniqueLoot.length} ${uniqueLoot.length === 1 ? "entry" : "entries"}</span></div>
        <div class="unique-loot-list">${uniqueLoot.map((item) => `<div class="unique-loot-row">${esc(item)}</div>`).join("")}</div>
      </section>` : "";
