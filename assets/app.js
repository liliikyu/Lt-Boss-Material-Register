(() => {
  ...
  function render() {
    ...
    $("dungeon-grid").innerHTML = visible.map((dungeon) => {
      ...
      const titleNote = `<div class="title-summary-row"><span><strong>Title</strong> ${titleCount}</span><span class="title-summary-names">${titleText}</span></div>`;
      ...
      const cardContentHtml = `
        ...
        ${monsterSummaryHtml}
        ${summaryHtml}
        ${isExpandable ? `<div class="dungeon-body">${monsterSectionHtml}${columnsHtml}</div>` : ""}
        ${titleNote}
      `;
