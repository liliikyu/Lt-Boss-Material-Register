  ...
  function render() {
    ...
    $("dungeon-grid").innerHTML = visible.map((dungeon) => {
      const itemPriority = (item) => {
        ...
        if (/\(Equipment\)\s*$/i.test(name)) return 0;
        if (/\(Event\)\s*$/i.test(name) && item.codex) return 1;
        if (/\(Event\)\s*$/i.test(name) && item.titleMaterial) return 2;
        return 3;
      };
      const items = [...(dungeon.items || [])].sort((a, b) => itemPriority(a) - itemPriority(b));
      ...
      const meta = dungeon.entriesPerDay ? `<span>${esc(dungeon.entriesPerDay)} ${Number(dungeon.entriesPerDay) === 1 ? "entry" : "entries"}/day</span>` : "";
