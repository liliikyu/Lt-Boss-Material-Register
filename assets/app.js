(() => {
  ...
  function render() {
    ...
    $("dungeon-grid").innerHTML = visible.map((dungeon) => {
      ...
      const isConsume = (item) => /\(Consume\)\s*$/i.test(String(item.name || ""));

      const cleanItemName = (name) => String(name || "")
        .replace(/\s*\((?:Equipment|Event|ETC|Consume)\)\s*$/i, "")
        .trim();
      ...
      const renderFlags = (item) => [
        isConsume(item) ? '<span class="flag consume" title="Consumable item">Consume</span>' : "",
      ...
      const itemCount = (dungeon.items || []).length;
      const isCollapsed = collapseState.has(dungeonKey) ? collapseState.get(dungeonKey) : itemCount >= LONG_CARD_THRESHOLD;
      return `<article class="dungeon-card${isCollapsed ? " collapsed" : ""}" data-category="${categoryFor(dungeon.level)}" data-dungeon-key="${esc(dungeonKey)}">
        <header class="dungeon-head">
          <button class="dungeon-toggle" type="button" aria-expanded="${isCollapsed ? "false" : "true"}" title="${isCollapsed ? "Expand" : "Collapse"} ${esc(dungeon.name)}">
  ...
  $("dungeon-grid").addEventListener("click", (event) => {
    ...
    collapseState.set(key, nextCollapsed);
    card.classList.toggle("collapsed", nextCollapsed);
    toggle.setAttribute("aria-expanded", String(!nextCollapsed));
    toggle.title = `${nextCollapsed ? "Expand" : "Collapse"} ${card.querySelector(".dungeon-name")?.textContent || "dungeon"}`;
  });
