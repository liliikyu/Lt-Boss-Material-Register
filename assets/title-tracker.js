(() => {
  ...
  function render(){
    ...
    $("title-table-body").innerHTML=visible.map((title)=>{
      const state=rowState(title.id); const req=numericRequired(title); const specialReq=specialRequirement(title); const mats=[...(title.materials||[])].slice(0,3);
      ...
      }).join("") : (specialReq ? `<div class="special-title-requirement">${esc(specialReq)}</div>` : '<div class="no-title-materials">No material tracking required.</div>');
