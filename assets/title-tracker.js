(() => {
    ...
    $("title-table-body").innerHTML=visible.map((title)=>{
      ...
      const materialRows=mats.length ? mats.map((mat,i)=>{
        ...
        return `<div class="material-progress-row">
          ...
          <label class="material-current-wrap"><span>Current</span><input class="current-input" type="number" min="0" step="1" inputmode="numeric" value="${esc(cur)}" data-title-id="${esc(title.id)}" data-index="${i}" aria-label="Current amount for ${esc(parsed.name || mat)}"></label>
          <div class="material-remaining ${ready ? "ready" : ""}" data-remaining-for="${esc(title.id)}:${i}"><span>Remaining</span><strong>${ready ? "✓" : esc(rem)}</strong></div>
        </div>`;
  ...
  $("title-table-body").addEventListener("input",(event)=>{
    ...
    const title=(D.titles||[]).find(t=>t.id===id); const req=numericRequired(title); const rem=Math.max(req-value,0); const target=document.querySelector(`[data-remaining-for="${CSS.escape(id+":"+index)}"]`); if(target){ const strong=target.querySelector("strong"); if(strong) strong.textContent=(req>0 && rem===0)?"✓":rem; target.classList.toggle("ready",req>0 && rem===0); }
  });
