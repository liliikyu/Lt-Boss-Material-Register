(() => {
  ...
  function elyRequirement(title){ const raw=title?.elyRequired; if(raw === null || raw === undefined || String(raw).trim() === "") return ""; const n=Number(raw); return Number.isFinite(n) ? `${n.toLocaleString()} Ely` : String(raw).trim(); }
  ...
  function render(){
    ...
    $("title-table-body").innerHTML=visible.map((title)=>{
      const state=rowState(title.id); const req=numericRequired(title); const specialReq=specialRequirement(title); const elyReq=elyRequirement(title); const mats=[...(title.materials||[])].filter((mat)=>!/^\s*[\d,]+\s+ely\s*$/i.test(String(mat||""))).slice(0,3);
      const materialRows=mats.length ? mats.map((mat,i)=>{
        const parsed=parseMaterial(mat);
        if(req === null){
          return `<div class="material-progress-row special-material-row">
            <div class="material-info"><span class="material-name">${esc(parsed.name || mat)}</span>${materialTypeBadge(parsed.type)}</div>
            ${specialReq ? `<div class="special-requirement-label">${esc(specialReq)}</div>` : ""}
          </div>`;
        }
      ...
      }).join("") : (specialReq ? `<div class="special-title-requirement">${esc(specialReq)}</div>` : (elyReq ? "" : '<div class="no-title-materials">No material tracking required.</div>'));
      const extraRequirement = elyReq ? `<div class="special-title-requirement ely-requirement" title="Ely requirement — no Current/Remaining tracking">${esc(elyReq)}</div>` : "";
