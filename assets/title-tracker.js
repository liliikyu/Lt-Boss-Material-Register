  ...
  function render(){
    ...
    $("title-table-body").innerHTML=visible.map((title)=>{
      ...
      return `<tr class="${state.complete ? "title-complete-row" : ""}" data-title-row="${esc(title.id)}">
        ...
        <td class="title-name-cell"><strong>${esc(title.title)}</strong><span class="title-dungeon-name">${esc(title.dungeon || "—")}${title.dungeonLevel ? ` (${esc(displayDungeonLevel(title.dungeonLevel))})` : ""}</span></td>
