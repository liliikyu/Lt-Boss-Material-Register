...
def build_dungeon_data(rows: list[list[str]]) -> dict:
    ...
    for col in range(2, width):
        ...
        if title_name and title_name not in {"-", "Not yet"}:
            ...
            # Support adjacent title columns for the same dungeon (e.g. BP/BQ).
            # If the current title column omits dungeon metadata but directly follows
            # another title column that had it, reuse that dungeon context for the title.
            ...
            if name:
                previous_title_context = (name, raw_level or None)
            else:
                # Only carry context across one immediately adjacent title column.
                previous_title_context = previous_title_context if previous_title_context else None
