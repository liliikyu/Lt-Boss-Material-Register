...
SHEET_NAME = "Dungeon Boss Material"
...
def add_item(items, name, *, codex=False, title=False, force_equipment=False):
    ...
    if existing is None:
        existing = {"_key": key, "name": name, "codex": False, "titleMaterial": False}
...
def build_data(rows: list[list[str]]) -> dict:
    if len(rows) < 36:
    ...
    drop_rows = [labels[f"Drop {n}"] for n in range(1, 11)] + [labels["Badge 5 Mats"]]
    ...
    for col in range(2, width):
        ...
        dungeons.append({
            ...
            "loot": [
                {"name": item["name"], "codex": item["codex"], "titleMaterial": item["titleMaterial"]}
                for item in items
            ],
        })
