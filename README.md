...
- `index.html` — lightweight redirect to the Dungeon Tracker
- `dungeons.html` — dungeon tracker
- `titles.html` — title tracker

GitHub Pages opens the Dungeon Tracker at the repository root. The Welcome/Guide is shown as an in-page popup and can be reopened with the `?` button.
...
Removed legacy, unreferenced favicon/logo/tutorial asset files. Current favicon is root `favicon.ico`; the header icon is `assets/favicon-purple.png`.
